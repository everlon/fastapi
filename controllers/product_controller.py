...
from database import get_db, log_product_view, get_product_views
...
async def list_products_endpoint(
    ...
    # Registra LOG da Visualização
    for product in products:
        await log_product_view(product.id, search)
...
async def get_product_by_id_endpoint(id: int, db: Session = Depends(get_db)):
    """
    ...
    # Registra LOG da Visualização
    # await log_product_view(product.id)

    views_count = await get_product_views(id)
...
async def update_product_endpoint(id: int, product_data: ProductUpdate, db: Session = Depends(get_db)):
    """
